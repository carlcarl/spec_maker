...
Spec_Maker is deployed on the following stack.
...
As documented you should create/import your Celery app in ``spec_maker/__init__.py`` so that you
can run the worker via::

    celery -A spec_maker worker

Additionally you will need to configure the project settings for Celery::

    # spec_maker.settings.staging.py
    import os
...
You will also need to add the ``BROKER_URL`` to the ``spec_maker.settings.production`` so
...
RabbitMQ user will be named spec_maker_<environment> and the vhost will be named spec_maker_<environment>
